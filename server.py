...
def danger():
    ...
    if len(request.form['name']) > 64:
        flash("Name cannot be greater than 64 charaters!")
    if len(request.form['comment']) < 1:
        flash("Comment cannot be blank!")
    if len(request.form['comment']) > 256:
        flash("Comment cannot be longer than 255 characters!")
    if len(request.form['name']) < 1 or len(request.form['comment']) < 1:
        return redirect('/')
    if len(request.form['name']) > 64 or len(request.form['comment']) > 255:
        return redirect('/')
